...
def random_music(dominant_emotion):
    try:
        ...
        # 獲取資料夾中的所有音樂檔案
        music_files = [f for f in os.listdir(music_folder) if f.endswith(('.mp3', '.wav', '.flac'))]
        if not music_files:
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if total_emotions >= 10:
            ...
            # 使用自定義字體顯示中文字符
            text = f"{emotion_translation[dominant_emotion]}: {percentage:.2f}%"
